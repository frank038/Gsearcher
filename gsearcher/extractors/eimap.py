...
def get_data_mail(ffile):
    try:
        ...
        if msg != "":
            ...
            #
            for el in ['to','cc','bc','ccn','bcc','from','subject']:
                _dd = mail[el]
                if _dd != None:
                    mail_content += el+": "+_dd+"\n"
            ############
            # mail_content += mail.get_body().as_string()
            mail_content += mail.get_body().get_content()
            ###
            # __payloads = mail.get_payload()
            # _type_text = None
            # _type_html = None
            # for el in __payloads:
                # if el.get_content_type() == "text/plain":
                    # _type_text = el
                # if el.get_content_type() == "text/html":
                    # _type_html = el
            # #
            # if _type_text != None:
                # mail_content += el.get_content()
            # elif _type_text != None:
                # mail_content += el.get_content()
            # else:
                # mail_content += mail.get_body().as_string()
            ###########
            ...
            # i = 0
            # for el in att:
                # "content type" = att[i].get_content_type())
                # i += 1
            #
